...
const VISIBILITY_TIMEOUT_SECS: usize = 300; // 5 minutes
...
impl<T: QueueItem + 'static> RedisQueue<T> {
    ...
    async fn get_async_connection(&self) -> Result<redis::aio::Connection> {
        ...
        self.redis_client.get_async_connection().await.map_err(|e| {
    ...
    pub fn enqueue(&self, item: T) -> Result<String> {
        ...
        let serialized = serde_json::to_string(&item).map_err(|e| Error::Serialization(e.to_string()))?;
        ...
        debug!("Enqueued item {} for token {}", operation_id, item.token_id());
        Ok(operation_id)
    }
    ...
    pub fn mark_completed(&self, item: &T) -> Result<()> {
        ...
        let serialized_item = serde_json::to_string(item).map_err(|e| Error::Serialization(e.to_string()))?;
        ...
        let removed: i32 = redis::cmd("LREM")
            ...
            .query(&mut conn)
            .map_err(|e| {
                error!("Failed to remove item {} from processing queue: {}", operation_id, e);
                Error::Redis(format!("Redis operation error: {}", e))
            })?;
        ...
        let _: () = pipe.query(&mut conn).map_err(|e| {
            error!("Failed to update metadata for completed item {}: {}", operation_id, e);
            Error::Redis(format!("Redis operation error: {}", e))
        })?;
    ...
    pub fn mark_failed(&self, item: &T, error_message: &str) -> Result<()> {
        ...
        if is_foreign_key_error {
            ...
            if token_failures >= 2 {
                warn!("Token {} has failed {} times, abandoning retry", token_id, token_failures + 1);
                ...
                pipe.cmd("HSET")
                    ...
                    .arg(Utc::now().to_rfc3339())
                    .arg("error")
                    .arg(format!("Abandoned after multiple FK failures: {}", error_message));
                ...
                error!("Item {} abandoned due to repeated foreign key failures for token {}", 
        ...
        if attempts >= MAX_RETRY_ATTEMPTS {
            ...
            error!("Item {} permanently failed after {} attempts: {}", 
            ...
            debug!("Item {} scheduled for retry in {} seconds: {}", 
    ...
    pub async fn mark_failed_async(&self, item: &T, error_message: &str) -> Result<()> {
        ...
        if attempts >= MAX_RETRY_ATTEMPTS {
            ...
            warn!("Item {} failed, will retry (attempt {}/{}): {}", 
    ...
    pub fn process_delayed_items(&self) -> Result<usize> {
        ...
        info!("Moved {} items from delayed queue to main queue", ready_items.len());
        Ok(ready_items.len())
    }
    ...
    pub fn handle_stuck_operations(&self, stuck_timeout_seconds: u64) -> Result<usize> {
        let mut conn = self.get_connection()?;
        let threshold = (Utc::now() - chrono::Duration::seconds(stuck_timeout_seconds as i64)).to_rfc3339();
        ...
        for serialized in processing_items {
            ...
            if let Some(started) = processing_started {
                if started < threshold {
                    ...
                    warn!("Found stuck item {}, processing since {}", operation_id, started);
                    
                    // Mark as failed and move back to main queue
                    if let Err(e) = self.mark_failed(&item, "Operation timed out") {
                        error!("Failed to mark stuck item {} as failed: {}", operation_id, e);
                        continue;
                    }
