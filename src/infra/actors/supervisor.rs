use super::{Actor, ActorRef, Command, Message, Query, QueryResult};
...
impl SupervisorActor {
    ...
    pub async fn restart_actor(&self, id: &str) -> Result<(), Error> {
        ...
        // Send start command
        let start_result = actor_ref
            .send(crate::actors::Message::Command(
                crate::actors::Command::Start,
            ))
            .await;
        ...
        match (stop_result, start_result) {
            ...
            (Err(e), Ok(_)) => {
                warn!(
                    "Actor '{}' stop command failed but start succeeded: {}",
                    id, e
                );
    ...
    pub async fn get_actor_status(&self, id: &str) -> Result<String, Error> {
        if let Some(actor) = self.get_actor(id).await {
            ...
            match rx.await {
                ...
                Ok(Ok(_)) => Err(Error::InvalidInput(
                    ...
                    match actor_ref.send(Message::Query(Query::GetStatus, tx)).await {
                        Ok(()) => {
                            ...
                            match tokio::time::timeout(Duration::from_secs(5), rx).await {
                                    ...
                                    if let Some(state) = states.get_mut(id) {
                                        ...
                                        if previous_state != is_running {
                                            if is_running {
                                                ...
                                                warn!(
                                                    "Actor {} has stopped running unexpectedly",
                                                    id
                                                );
                                                ...
                                                if state.failure_count >= 3 {
                                                    ...
                                                    if ["market", "strategy", "risk", "execution"]
                                                        .contains(&id.as_str())
                                                    {
                                ...
                                Err(_) => {
                                    error!(
                                        "Actor {} status check timed out - actor may be hung",
                                        id
                                    );
    ...
    pub async fn get_health_report(&self) -> Result<serde_json::Value, Error> {
        ...
        for (id, _) in actors.iter() {
            ...
            let status = if actor_state.is_running {
                "running"
            } else {
                "stopped"
            };
    ...
    pub async fn force_reconnect_actor(&self, actor_type: &str) -> Result<(), Error> {
        info!(
            "Supervisor initiating emergency reconnection procedure for {}",
            actor_type
        );
        ...
        info!(
            "Found {} actors to reconnect for type {}",
            matching_actors.len(),
            actor_type
        );
        ...
        for (id, actor_ref) in matching_actors {
            ...
            info!(
                "☢️ EMERGENCY RECOVERY: Force restarting actor '{}' due to communication failures",
                id
            );
            ...
            match actor_ref.send(Message::Command(Command::Start)).await {
                Ok(_) => {
                    info!(
                        "Successfully restarted actor '{}' during emergency recovery",
                        id
                    );
                ...
                Err(e) => {
                    error!(
                        "Failed to restart actor '{}' during emergency recovery: {}",
                        id, e
                    );
                }
            }
    ...
    pub async fn regenerate_communication_channels(&self, actor_type: &str) -> Result<(), Error> {
        info!(
            "Regenerating communication channels for actor type: {}",
            actor_type
        );
    ...
    pub async fn find_supervisor() -> Option<Arc<Self>> {
        ...
        // not modifying it. Once::call_once ensures it's initialized safely.
        // We'll only get None if it's not initialized yet or initialization failed.
        unsafe {
            let ptr = std::ptr::addr_of!(GLOBAL_SUPERVISOR_INSTANCE);
            match &*ptr {
                // Dereference the raw pointer
                Some(supervisor_arc) => Some(supervisor_arc.clone()),
                None => None,
            }
        }
    }
    ...
    pub async fn establish_actor_connections(
        &self,
        connections: &[(&str, &str)],
            ...
            if let Some(target_ref) = actors.get(*target) {
                    ...
                    .await
                {
                    Ok(_) => {
                        ...
                        info!(
                            "Successfully established actor connection: {} → {}",
                            source, target
                        );
                    }
                    Err(e) => {
                        ...
                        error!(
                            "Failed to establish actor connection {} → {}: {}",
                            source, target, e
                        );
                ...
                warn!(
                    "Target actor '{}' not found - connection from {} skipped",
                    target, source
                );
            }
        }
        ...
        if success_count == total_count {
            info!(
                "All actor connections ({}/{}) successfully established",
                success_count, total_count
            );
        } else {
            warn!(
                "Some actor connections failed: {}/{} successful",
                success_count, total_count
            );
        }
    ...
    pub async fn establish_database_connections(
        &self,
        database_actor_id: &str,
    ) -> Result<HashMap<String, bool>, Error> {
        ...
        let database_ref = match actors.get(database_actor_id) {
            ...
            None => {
                error!(
                    "Database actor '{}' not found - cannot set up event logging",
                    database_actor_id
                );
                return Err(Error::NotFound(format!(
                    "Database actor not found: {}",
                    database_actor_id
                )));
            }
        };
        ...
        match msg {
            ...
            Message::Command(cmd) => match cmd {
                ...
                Command::UpdateConfig(config) => {
                    ...
                    if let Some(interval) =
                        config.get("health_check_interval").and_then(|v| v.as_u64())
                    {
                ...
                Command::MaintenanceDb => {
                    ...
                    match self.get_actor("database").await {
                        Some(db_actor) => db_actor
                            .send(Message::Command(Command::MaintenanceDb))
                            .await
                            .map_err(|e| {
                                Error::Task(format!("Failed to forward maintenance command: {}", e))
                            }),
                        None => {
                ...
                Command::StartMaintenanceScheduler => {
                    ...
                    match self.get_actor("database").await {
                        Some(db_actor) => db_actor
                            .send(Message::Command(Command::StartMaintenanceScheduler))
                            .await
                            .map_err(|e| {
                                Error::Task(format!(
                                    "Failed to forward maintenance scheduler command: {}",
                                    e
                                ))
                            }),
                        None => {
            ...
            Message::Query(query, responder) => match query {
                Query::GetStatus => {
                    ...
                    let status = format!(
                        "SupervisorActor running: {}, monitoring {} actors",
                        self.running, actors_count
                    );
                    responder
                        .send(Ok(QueryResult::Status(status)))
                        .map_err(|e| {
                            Error::InvalidInput(format!("Failed to send status response: {:?}", e))
                        })
                }
                Query::GetMetrics => {
                    ...
                    responder
                        ...
                        .map_err(|e| {
                            Error::InvalidInput(format!("Failed to send metrics response: {:?}", e))
                        })
                }
                _ => responder
