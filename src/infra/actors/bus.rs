...
use super::ActorRef;
use super::{DatabaseEvent, Event, ExecutionEvent, MarketEvent, RiskEvent, StrategyEvent};
...
impl MessageBus {
    ...
    pub fn instance() -> Arc<MessageBus> {
        ...
        // and Once ensures that concurrent access is safe.
        unsafe {
            let ptr = std::ptr::addr_of!(GLOBAL_MESSAGE_BUS_INSTANCE);
            match &*ptr {
                // Dereference the raw pointer to get an &Option<Arc<MessageBus>>
                Some(bus_arc) => {
                    let bus_id = format!("{:p}", Arc::as_ptr(bus_arc));
                    debug!("Retrieving global MessageBus instance [id: {}]", bus_id);
                    trace!(
                        "Global MessageBus pointer address: {:p}",
                        Arc::as_ptr(bus_arc)
                    );
                    bus_arc.clone()
                }
                None => {
                    // This case should be impossible due to Once::call_once
                    // but to be safe and satisfy the compiler for unsafe block:
                    error!("Critical: GLOBAL_MESSAGE_BUS_INSTANCE is None after Once::call_once!");
                    panic!("Global MessageBus not initialized!");
                }
            }
        }
    }
    ...
    async fn perform_maintenance(&self) -> usize {
        ...
        async fn async_fn_impl(bus: &MessageBus) -> usize {
            ...
            for (event_type, subs) in subscribers.iter_mut() {
                ...
                subs.retain(|sub| {
                    ...
                    let status_check_event = match event_type.as_str() {
                        ...
                        _ => Event::Market(MarketEvent::StatusCheck), // Default to market
                    };
                ...
                if removed_count > 0 {
                    info!(
                        "MessageBus maintenance: Removed {} dead subscriber(s) from {} events",
                    ...
                    if subs.is_empty() {
                        info!(
                            "All subscribers removed from {} event bus - will initiate recovery",
                            event_type
                        );
            ...
            for event_type in recovery_needed {
                ...
                if let Some(subs) = direct_subscribers.get_mut(&event_type) {
                    if subs.is_empty() {
                        ...
                        let (recovery_tx, _new_receiver) = mpsc::channel::<Event>(500); // Prefixed new_receiver
            ...
            if total_removed > 0 {
                debug!(
                    "Removed {} subscribers - consider running maintenance again soon",
                    total_removed
                );
            }
            total_removed
    ...
    pub async fn subscribe(
        ...
        info!(
            "Added new subscriber to {} events (id: {})",
    ...
    pub async fn unsubscribe(
        ...
        trace!(
            "MessageBus: Removing subscriber for event type: {}",
            event_type
        );
        let mut subscribers = self.subscribers.write().await;
        if let Some(subs) = subscribers.get_mut(event_type) {
            ...
            debug!(
                "Unsubscribed from {} events (remaining: {})",
        ...
        if let Some(info) = token_events.get(token_id) {
            if info.last_event_type == event_type {
                ...
                if elapsed < self.deduplication_window {
                    debug!(
                        "Deduplicating event for token {}: {} (elapsed: {:?})",
    ...
    async fn update_token_event(&self, token_id: &str, event_type: &str) {
        ...
        token_events.insert(
            token_id.to_string(),
            TokenEventInfo::new(event_type.to_string()),
        );
    }
    ...
    fn should_deduplicate_buy_order(&self, token_id: &str) -> bool {
        ...
        if let Some(last_time) = locks.get(token_id) {
            ...
            if elapsed < Duration::from_secs(120) {
                if elapsed.as_secs() < 5 {
                    ...
                    info!(
                        "🔄 Deduplicating BUY order for {} within {} seconds - skipping",
                        token_id,
                        elapsed.as_secs()
                    );
                }
        ...
        locks.retain(|_, timestamp| now.duration_since(*timestamp) < Duration::from_secs(180));
            ...
            if let Event::Execution(ExecutionEvent::OrderExecuted { signal, .. }) = &event {
                if *signal == crate::trading::strategy::Signal::Buy {
                    if self.should_deduplicate_buy_order(token_id) {
                        info!(
                            "🚫 Suppressed duplicate BUY OrderExecuted event for {}",
                            token_id
                        );
                        return Ok(());
                    } else {
                        info!(
                            "✅ Processing unique BUY OrderExecuted event for {}",
                            token_id
                        );
                    }
                }
    ...
    async fn publish_internal(&self, event: Event) -> Result<(), Error> {
        let event_type = match &event {
            ...
            Event::DexTransaction(_) => "dex_transaction",
        };
        ...
        let should_attempt_recovery = {
            ...
            if elapsed > backoff_duration {
                ...
                debug!(
                    "Skipping recovery for {} events - attempted {:.1}s ago (backoff: {:.1}s)",
                    event_type,
                    elapsed.as_secs_f32(),
        ...
        // --- Refactor Start ---
        let subscribers_to_notify: Vec<mpsc::Sender<Event>>;

        {
            // Scope for the first read lock
            let subscribers_guard_initial_check = self.subscribers.read().await;

            if !subscribers_guard_initial_check.contains_key(event_type) {
                ...
                warn!(
                    "No subscribers found for {} events - event discarded",
                    event_type
                );
                return Ok(());
            }

            let needs_maintenance = subscribers_guard_initial_check
                .get(event_type)
                .map(|subs| subs.iter().any(|s| s.consecutive_failures >= 100))
                .unwrap_or(false);

            drop(subscribers_guard_initial_check); // Explicitly drop before potential await

            if needs_maintenance {
                warn!("Detected subscribers with extreme failure counts - initiating emergency maintenance");
                let removed = self.perform_maintenance().await; // .await is here
                info!("Emergency maintenance removed {} subscribers", removed);

                // Re-acquire lock to check if subscribers still exist and get them
                let subscribers_guard_after_maintenance = self.subscribers.read().await;
                if !subscribers_guard_after_maintenance
                    .get(event_type)
                    .map_or(false, |subs| !subs.is_empty())
                {
                    debug!(
                        "No subscribers left after maintenance for {} - message dropped",
                        event_type
                    );
                    return Ok(());
                }
                subscribers_to_notify = subscribers_guard_after_maintenance
                    .get(event_type)
                    .map(|subs| subs.iter().map(|s| s.sender.clone()).collect())
                    .unwrap_or_default();
                // subscribers_guard_after_maintenance drops here
            } else {
                // Acquire lock again if maintenance was not needed, to get subscribers
                let subscribers_guard = self.subscribers.read().await;
                subscribers_to_notify = subscribers_guard
                    .get(event_type)
                    .map(|subs| subs.iter().map(|s| s.sender.clone()).collect())
                    .unwrap_or_default();
                // subscribers_guard drops here
            }
        } // End of scope for read locks related to getting subscribers_to_notify

        if let Event::Risk(RiskEvent::PositionClosed { token_id, .. }) = &event {
            debug!("DIAGNOSTICS: Found {} subscribers for {} events when publishing PositionClosed for {}",
                  subscribers_to_notify.len(), event_type, token_id);
        }
        // --- Refactor End ---
        ...
        match &event {
            Event::Market(MarketEvent::PriceUpdate {
                ...
                trace!(
                    "Publishing market price update for {}: ${:.4}",
                    token_id,
                    price
                );
            }
            Event::Strategy(StrategyEvent::Signal {
                ...
                trace!(
                    "Publishing strategy signal {:?} for {} with {:.1}% confidence",
                    signal,
                    token_id,
                    confidence * 100.0
                );
            ...
            Event::Risk(RiskEvent::RiskAssessment {
                ...
                trace!(
                    "Publishing risk assessment for {} with signal {:?}",
                    token_id,
                    signal
                );
            }
            Event::Execution(ExecutionEvent::OrderExecuted {
                ...
                trace!(
                    "Publishing execution event for {}: {:?}",
                    canonical_token_id,
                    signal
                );
            }
        ...
        for (index, sender) in subscribers_to_notify.iter().enumerate() {
        ...
        if !failed_indices.is_empty() {
            let mut perform_maintenance_after_failure_handling = false;
            let mut perform_supervisor_restart_after_failure_handling = false;

            {
                // Scope for write lock
                let mut subscribers_guard_write = self.subscribers.write().await;

                if let Some(subs) = subscribers_guard_write.get_mut(event_type) {
                    // Update failure counts for failed subscribers
                    for &index in &failed_indices {
                        if index < subs.len() {
                            // Ensure index is valid, subscribers list might have changed
                            ...
                            if failure_count >= 25 && !subs[index].is_reconnecting {
                                ...
                                warn!(
                                    "Scheduling reconnection for {} actor (event type: {})",
                                    subs[index].actor_type.clone(),
                                    event_type.to_string()
                                );
                                // Set flag to perform maintenance outside the lock
                                perform_maintenance_after_failure_handling = true;
                                // No return Ok(()) here, let the loop continue for other subscribers.
                    ...
                    // Perform recovery if we had multiple failures and all subscribers failed
                    if success_count == 0
                        && should_attempt_recovery
                        && !subscribers_to_notify.is_empty()
                    {
                        warn!("Failed to publish {} event to any subscribers (all {} failed) - initiating recovery",
                              event_type, subscribers_to_notify.len());
                        error!("RECOVERY ALERT: Critical failure in {} event bus - initiating direct recovery process", event_type);
                        // Set flag to request supervisor restart outside the lock
                        perform_supervisor_restart_after_failure_handling = true;
                    } else {
                        // Reset consecutive failures for subscribers that succeeded (if any)
                        for index in 0..subs.len() {
                            if !failed_indices.contains(&index) {
                                if subs[index].is_reconnecting {
                                    debug!(
                                        "Subscriber #{} successfully reconnected after failures",
                                        index
                                    );
                ...
                // subscribers_guard_write drops here
            }

            // Perform actions outside the write lock
            if perform_maintenance_after_failure_handling {
                let _ = self.perform_maintenance().await;
                // If maintenance was performed due to critical subscriber failure,
                // it might be prudent to return or re-evaluate before proceeding further with this event.
                // For now, we'll let it be, assuming maintenance cleans up the bad subscriber.
            }

            if perform_supervisor_restart_after_failure_handling {
                if let Err(e) = Self::request_supervisor_restart(event_type).await {
                    error!(
                        "Failed to initiate supervisor recovery for {}: {}",
                        event_type, e
                    );
                }
                let _ = self.perform_maintenance().await; // Clean up after recovery attempt
                {
                    let mut attempt_counts = RECOVERY_ATTEMPT_COUNTS.lock().unwrap();
                    attempt_counts.insert(event_type.to_string(), 0); // Reset on successful recovery attempt
                }
                // If all subscribers failed and recovery was attempted, it's reasonable to return.
                // The event likely wasn't delivered to anyone critical.
                return Ok(());
            }

            let failed = subscribers_to_notify.len() - success_count;
            if failed > 0 || success_count > 0 {
                // Log if there were any attempts
                debug!(
                    "Published {} event to {}/{} subscribers ({} failed)",
                    event_type,
                    success_count,
                    subscribers_to_notify.len(),
                    failed
                );
            }
            ...
            trace!(
                "Successfully published {} event to all {} subscribers",
                event_type,
                success_count
            );
        }
    ...
    async fn request_supervisor_restart(event_type: &str) -> Result<(), Error> {
        ...
        info!(
            "Requesting supervisor to restart actors for event type: {}",
            event_type
        );
        ...
        if let Some(supervisor_ref) = &supervisor {
            ...
            for actor_type in &actor_types {
                ...
                if let Err(e) = supervisor_ref.force_reconnect_actor(actor_type).await {
                    ...
                    info!(
                        "Successfully requested restart of {} via supervisor",
                        actor_type
                    );
        ...
        warn!(
            "Attempting direct channel recovery for {} without supervisor",
            event_type
        );
        ...
        let recovery_message = format!(
            "RECOVERY: Attempting to repair {} communication channels",
            event_type
        );
        let recovery_announcement =
            Event::Market(MarketEvent::SupervisorRecoveryRequest(recovery_message));
    ...
    async fn recreate_subscribers_for_event_type(event_type: &str) -> Result<(), Error> {
        ...
        if let Some(subs) = subscribers.get_mut(event_type) {
            ...
            if count > 0 {
                warn!(
                    "Removing {} stuck subscribers from {} event bus",
                    count, event_type
                );
                subs.clear();
        ...
        info!(
            "Creating recovery channel for {} events, actor type: {}",
            event_type, actor_type
        );

        if let Err(e) = bus
            .subscribe(event_type.to_string(), recovery_tx.clone())
            .await
        {
            error!(
                "Failed to create recovery channel for {}: {}",
                event_type, e
            );
            return Err(e);
        }
    ...
    pub async fn create_subscription_pipeline(
        ...
        for attempt in 1..=max_retries {
            match self
                .subscribe(source_event_type.to_string(), sender.clone())
                .await
            {
                Ok(_) => {
                    info!(
                        "Successfully subscribed to {} events (attempt {})",
                        source_event_type, attempt
                    );
                    success = true;
                    break;
                }
                Err(e) => {
                    error!(
                        "Attempt {} failed to subscribe to {} events: {}",
                        attempt, source_event_type, e
                    );
                    if attempt == max_retries {
                        return Err(Error::Task(format!(
                            "MessageBus subscription failed after {} attempts: {}",
                            max_retries, e
                        )));
                    }
                    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
            ...
            tokio::spawn(async move {
                ...
                while let Some(event) = receiver.recv().await {
                    ...
                    if crate::domain::trading::execution::bot::is_forced_shutdown() {
                        info!(
                            "Force shutdown detected, terminating {}→Actor event loop",
                            event_type
                        );
                        break;
                    }
                ...
                warn!(
                    "{}→Actor event forwarder exited after {} events",
                    event_type, count
                );
            });
        }
        ...
        if success {
            ...
            Err(Error::Task(format!(
                "Failed to establish subscription to {} events",
                source_event_type
            )))
        }
    }
    ...
    pub async fn create_database_subscriptions(
        ...
        if database_actor_ref.is_none() {
            return Err(Error::InvalidInput(
                "No database actor reference provided".to_string(),
            ));
        }
        ...
        for event_type_str in &event_types {
            ...
            match self
                .create_subscription_pipeline(
                    event_type_str, // Use the new variable name
            ...
            {
                Ok(_) => {
                    success_map.insert(event_type_str.to_string(), true); // Use the new variable name
                    debug!(
                        "Successfully set up database subscription to {} events",
                        event_type_str // Use the new variable name
                    );
                }
                Err(e) => {
                    success_map.insert(event_type_str.to_string(), false); // Use the new variable name
                    warn!(
                        "Failed to set up database subscription to {} events: {}",
                        event_type_str,
                        e // Use the new variable name
                    );
        ...
        for event_type_str in &event_types {
            ...
            let sub_count = self.get_subscriber_count(event_type_str).await; // Use the new variable name
            info!(
                "DIAGNOSTICS: {} event bus has {} subscribers",
                event_type_str,
                sub_count // Use the new variable name
            );
        }
