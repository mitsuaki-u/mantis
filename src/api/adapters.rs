...
pub fn convert_coingecko_to_token_data(coin_data: &Value) -> Result<TokenData, Error> {
    ...
    if let Some(market_cap_rank) = coin_data["market_cap_rank"].as_u64() {
        token_data.market_cap_rank = Some(market_cap_rank as usize);
    }
